...
void access_shared_resource(int task_id, const char* task_name, gpio_num_t led_pin) {
    ...
    /* Mutex operations commented out for race-condition demonstration.
       The critical section below now runs without taking the mutex so
       you can observe race conditions and data corruption. */
    ESP_LOGW(TAG, "[%s] ⚠️  Mutex DISABLED - entering critical section WITHOUT protection", task_name);
    ...
    // === CRITICAL SECTION BEGINS (UNPROTECTED) ===
    ...
    // Modify shared data (UNPROTECTED)
    ...
    // === CRITICAL SECTION ENDS (UNPROTECTED) ===

    // Turn off LEDs
    gpio_set_level(led_pin, 0);
    gpio_set_level(LED_CRITICAL, 0);

    ESP_LOGW(TAG, "[%s] (NO MUTEX) Completed critical section", task_name);
}
...
void app_main(void) {
    ...
    // Create tasks with different priorities (adjusted per request)
    xTaskCreate(high_priority_task, "HighPri", 3072, NULL, 2, NULL); // reduced priority
    xTaskCreate(medium_priority_task, "MedPri", 3072, NULL, 3, NULL);
    xTaskCreate(low_priority_task, "LowPri", 3072, NULL, 5, NULL);  // increased priority
    ...
    ESP_LOGI(TAG, "  High Priority Task: 2 (adjusted)");
        ESP_LOGI(TAG, "  Priority Inversion: 4"); 
        ESP_LOGI(TAG, "  Medium Priority:    3");
    ESP_LOGI(TAG, "  Low Priority:       5 (adjusted)");
